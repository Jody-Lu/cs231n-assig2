...
def batchnorm_backward_alt(dout, cache):
  ...
  dmu = np.sum(dxhat * (-1) / sqrtvar, axis = 0) + dvar * np.sum(-2 * (xmu), axis = 0) / N
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  #############################################################################
  for n in xrange(0, N): # nth data
    pad_x[n] = np.pad(x[n], ((0, 0), (pad, pad), (pad, pad)), 'constant', constant_values=0)
    for f in xrange(0, F): # fth filter
      for i, x_i in zip(xrange(0, H_out), xrange(0, H, stride)):
        for j, x_j in zip(xrange(0, W_out), xrange(0, W, stride)):
          out[n][f][i][j] = np.sum(pad_x[n, :, x_i:(x_i + HH), x_j:(x_j + WW)] * w[f])
      out[n][f] += b[f]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
